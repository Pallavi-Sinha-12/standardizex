...
The delta tables are stored in DBFS and this code snippet can be used in Databricks notebook.
'''
...
supplier_df.write.format("delta").mode("overwrite").save("dbfs:/FileStore/project/standardizex/supplier")
Product_df.write.format("delta").mode("overwrite").save("dbfs:/FileStore/project/standardizex/Product")
    ...
    "dependency_data_products" : [
        {
            ...
            "location" : "dbfs:/FileStore/project/standardizex/Product"
        }
    ],
    "schema" : {
        "source_columns" : [
        ...
        "new_columns" : [
            {
                "name" : "Product_ID",
                "data_type" : "string",
                "sql_transformation" : "MERGE INTO {temp_std_dp_path} dest USING delta.`dbfs:/FileStore/project/standardizex/Product` src ON dest.Product_Name = src.Product_Name WHEN MATCHED THEN UPDATE SET dest.Product_ID = src.Product_ID"
            }
        ]
    },
...
config_path = "dbfs:/FileStore/project/standardizex/config-1.json"
...
raw_dp_path = "dbfs:/FileStore/project/standardizex/supplier"
temp_std_dp_path = "dbfs:/FileStore/project/standardizex/Product_Supplier_temp"
std_dp_path = "dbfs:/FileStore/project/standardizex/Product_Supplier"
