...
The delta tables are stored locally and accessed using the local file system path.
'''
    ...
    "dependency_data_products" : [
        {
            ...
            "location" : "<local path to the Product DP>"
        }
    ],
    "schema" : {
        "source_columns" : [
        ...
        "new_columns" : [
            {
                "name" : "Product_ID",
                "data_type" : "string",
                "sql_transformation" : "MERGE INTO {temp_std_dp_path} dest USING delta.`<local path of Product DP>` src ON dest.Product_Name = src.Product_Name WHEN MATCHED THEN UPDATE SET dest.Product_ID = src.Product_ID"
            }
        ]
    },
